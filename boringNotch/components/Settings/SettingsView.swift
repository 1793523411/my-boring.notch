        ...
        Form {
            ...
            Section {
                Defaults.Toggle(key: .showMusicPlayer) {
                    Text("Show music player in notch")
                }
                Defaults.Toggle(key: .musicPlayerPrivacyMode) {
                    HStack {
                        Text("Music player privacy mode")
                        Spacer()
                        Image(systemName: "lock.shield.fill")
                            .foregroundColor(.gray)
                    }
                }
                .disabled(!Defaults[.showMusicPlayer])
                Defaults.Toggle(key: .showSystemControls) {
                    Text("Show volume and brightness controls")
                }
            } header: {
                HStack {
                    Text("Notch Content")
                ...
                Text("Privacy mode hides song title, artist and album art, showing only playback controls.")
                    .font(.caption)
                    .foregroundColor(.secondary)
            }
...
struct Advanced: View {
    ...
    @State private var volcAccessKey: String = ""
    @State private var volcSecretKey: String = ""
    @State private var translationConfigSaved = false
    let icons: [String] = ["logo2"]
    @State private var selectedIcon: String = "logo2"
    ...
    var body: some View {
        Form {
            ...
            Section {
                TextField("Access Key", text: $volcAccessKey)
                    .textFieldStyle(.roundedBorder)
                SecureField("Secret Key", text: $volcSecretKey)
                    .textFieldStyle(.roundedBorder)
                HStack {
                    Button("Save Keys") {
                        VolcengineTranslationService.shared.updateCredentials(
                            accessKey: volcAccessKey,
                            secretKey: volcSecretKey
                        )
                        translationConfigSaved = true
                    }
                    .disabled(volcAccessKey.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty || volcSecretKey.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty)

                    Button("Reload") {
                        loadTranslationCredentials()
                    }

                    Spacer()

                    if translationConfigSaved {
                        Text("Saved")
                            .font(.caption)
                            .foregroundStyle(.secondary)
                    }
                ...
                Text("Translator uses Volcengine Machine Translation with your Access Key and Secret Key.")
                    .multilineTextAlignment(.trailing)
                    .foregroundStyle(.secondary)
                    .font(.caption)
            }
    ...
    private func loadTranslationCredentials() {
        volcAccessKey = VolcengineTranslationService.shared.currentAccessKey()
        volcSecretKey = VolcengineTranslationService.shared.currentSecretKey()
        translationConfigSaved = false
    }
}
